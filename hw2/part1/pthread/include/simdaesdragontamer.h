...
static inline __m256i aesdragontamer_r(aesdragontamer_state *state)
{
    ...
    return _mm256_insertf128_si256(_mm256_castsi128_si256(penultimate2),
                                   penultimate1, 1);
}

static inline void aesdragontamer_seed_r(uint64_t seed1, uint64_t seed2,
                                         aesdragontamer_state *state)
{
    state->increment =
...
static inline __m256i aesdragontamer()
{
    return aesdragontamer_r(&g_aesdragontamer_state);
}

static __m256i avx_dragonrandombound_epu32(__m256i randomvals,
                                           __m256i upperbound)
{
    /* four values */
    __m256i evenparts =
...
void aesdragontamer_shuffle32(aesdragontamer_state *key, uint32_t *storage,
                              uint32_t size)
{
    uint32_t i;
    uint32_t randomsource[8];
    __m256i interval =
        _mm256_setr_epi32(size, size - 1, size - 2, size - 3, size - 4,
