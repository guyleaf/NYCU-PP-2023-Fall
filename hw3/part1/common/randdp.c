...
double randlc(double *x, double a)
{
    ...
    //  precision integers in the range (1, 2^46).  The returned value RANDLC is
    //  normalized to be between 0 and 1, i.e. RANDLC = 2^(-46) * x_1.  X is
    //  updated to contain the new seed x_1, so that subsequent calls to RANDLC
    //  using the same arguments will generate a continuous sequence.
    //
...
void vranlc(int n, double *x, double a, double y[])
{
    ...
    //  numbers before repeating.  The argument A is the same as 'a' in the
    //  above formula, and X is the same as x_0.  A and X must be odd double
    //  precision integers in the range (1, 2^46).  The N results are placed in
    //  Y and are normalized to be between 0 and 1.  X is updated to contain the
    //  new seed, so that subsequent calls to VRANLC using the same arguments
    //  will generate a continuous sequence.  If N is zero, only initialization
    //  is performed, and the variables X, A and Y are ignored.
    //
