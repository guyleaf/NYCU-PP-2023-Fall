...
void initValue(float *values1, float *values2, double *value3, float *output,
               unsigned int N);

extern void test1(float *a, float *b, float *c, int N);
extern void test2(float *__restrict a, float *__restrict b, float *__restrict c,
                  int N);
    ...
    while ((opt = getopt_long(argc, argv, "st:?", long_options, NULL)) != EOF)
    {
        switch (opt)
        {
            ...
            case 't':
                ...
                {
                    cout << "Error: test" << whichTestToRun
                         << "() is not available.\n";
                    return -1;
                }
                break;
...
void initValue(float *values1, float *values2, double *values3, float *output,
               unsigned int N)
{
